...
def apply_env_overrides(config: Config) -> Config:
    ...
    if os.environ.get("BOT_WHITELIST"):
        try:
            ...
            whitelist_str = os.environ["BOT_WHITELIST"]
            if "," in whitelist_str:
                config.bot.bot_whitelist = [int(x.strip()) for x in whitelist_str.split(",") if x.strip()]
            else:
                config.bot.bot_whitelist = [int(whitelist_str)]
